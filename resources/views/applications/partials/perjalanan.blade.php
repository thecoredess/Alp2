...
<div class="space-y-8">
    ...
    <div class="card overflow-hidden">
        <div class="border-b border-gray-100 bg-gradient-to-r from-navy-50 to-royal-50 px-6 py-4">
            ...
            <ol class="relative space-y-0">
                @foreach ($stages as $index => $stage)
                    @php
                        ...
                        $isCurrent = $index === $currentIndex && ! $isComplete($stage);
                        $isLast = $index === $totalCount - 1;
                    @endphp
                    <li class="relative flex gap-4 pb-12 last:pb-0">
                        @unless ($isLast)
                            <span @class([
                                'absolute left-[15px] top-8 h-[calc(100%-8px)] w-0.5',
                                'bg-green-400' => $stage['done'] || $isSkipped,
                                'bg-gray-200' => ! $stage['done'] && ! $isSkipped,
                            ]) aria-hidden="true"></span>
                        @endunless

                        <span @class([
                            'relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ring-4 ring-white',
                            'bg-green-600 text-white shadow-sm shadow-green-200' => $stage['done'],
                            'bg-gray-300 text-white shadow-sm' => $isSkipped,
                            'bg-royal-500 text-white shadow-md shadow-royal-200 ring-royal-100' => $isCurrent,
                            'bg-gray-100 text-gray-400' => ! $stage['done'] && ! $isCurrent && ! $isSkipped,
                        ])>
                            @if ($stage['done'])
                        ...
                        <div @class([
                            ...
                            'border-green-200 bg-green-50/60' => $stage['done'],
